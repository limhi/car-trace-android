 ...
 */
function validateImageUrl(imagePath, checkExtension) {
...
exports.loadImage = function(url, options, callback) {
    ...
    filename = url.substring(n+1, l);
    
    Ti.API.info('in cacheImage, url = ' + url);
    Ti.API.info('in cacheImage, path = ' + path);
    Ti.API.info('in cacheImage, filename = ' + filename);
